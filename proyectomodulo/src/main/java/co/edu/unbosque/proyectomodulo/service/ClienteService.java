...
/**
 * Servicio que gestiona las operaciones CRUD y autenticación para {@link Cliente}.
 *
 * @version 1.0
 */
@Service
public class ClienteService implements CRUDOPERATION<ClienteDTO> {
    ...
    public String getAll() {
        if (!adminService.isLoggedadmin()) return null;
        Gson gson = new Gson();
    ...
    public int deleteById(Long id) {
        if (!adminService.isLoggedadmin()) return 2;
    ...
    public int updateById(Long id, ClienteDTO dataC) {
        ...
        if (encontradoUsuario.isPresent() && encontradoCedula.isPresent()) return 5;
        if (encontradoUsuario.isPresent()) return 3;
        if (encontradoCedula.isPresent())  return 4;
    ...
    @Override
    public long count() { return clienteRep.count(); }

    @Override
    public boolean exist(Long id) { return clienteRep.existsById(id); }

    public Cliente getClienteLogueado() { return clienteLogueado; }

    public String getClienteStatus(Long id) {
        Optional<Cliente> encontrado = clienteRep.findById(id);
        if (encontrado.isEmpty()) return "NORMAL";
